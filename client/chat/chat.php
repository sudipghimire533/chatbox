...
<body onload="ready()">
  ...
  <div id="MessageBoxContainer"style='margin-right: 0;margin-left: 0;'>
    <div id="WindowsBtn"style="margin-bottom:10px;position: fixed;display: inline-block;background-color: var(--color-mid); width: calc(var(--messageBoxWidth) - 10px);z-index: 1;border: 1px solid var(--super-light);">
      <i class='fas fa-times-circle'id='messageCloser'style='cursor: pointer;color: red;'onclick='closeMessage();'></i>
      <i class='fas fa-minus-circle'id='messageMinimizer'style='cursor: pointer;color: green;'></i>
      <i class='fas fa-plus-circle'id='messageMaximizer'style='cursor: pointer;color: yellow;'onclick='maximizeMessage(this)'></i>
      <strong class="title"id='PersonName'><?php echo $_COOKIE['LastTalkWith']; ?></strong>
      <i class='fas fa-chevron-left'id='back'style='cursor: pointer;color: var(--super-dark);float: right;font-size: 14px;'onmouseover="this.style.textDecoration='underline';this.style.color='var(--super-light)';"onmouseout="this.style.textDecoration='none';this.style.color='var(--super-dark)'"onclick='closeMessage();'></i>
    </div>
    <div id="MessagesParent">
      <div id="Messages">
        ...
        if(file_exists($filePath)){
          ...
          unset($content, $i, $totalMsg, $readLine);
        }
        else{
          $content = file('../Users/ChatBox/Messages/Messages.php');
          echo $content[0];
          unset($content);
        }
        unset($cookieName, $userName, $LastTalkWith, $filePath);
          ?>
      </div>
        <div id="showCasePrev">
